...
const InterviewNotice = (props) => {
  ...
  const contents = [
    ...
    (<span>- {announceString.slice(0, -5)} 인터뷰 대상자 <strong>발표 이후에는 희망 시간대 변경이 어려우니,</strong>
      {`변경을 원하시는 경우 발표 이전에 본 단체 제 ${batch}기 리크루팅 공식 계정 (recruiting${batch}@freemed.or.kr) 으로 변경 신청 메일을 보내주시기 바랍니다.`}</span>),
    (<span>- 본 단체에서는 인터뷰 대상자 분들의 시간적 편의를 최대한 고려하겠습니다만,
      불가피한 경우 원하시는 시간대에 인터뷰 시간이 배정되지 못할 수도 있음을 너그러이 양해 부탁 드립니다.</span>),
  ...
  return (
    <div className={cx('interview-notice-form')}>
      ...
      {contents.map((content, index) => (
        <li key={`Content__${index}`} className={cx(index === 0 ? 'line intro-line' : 'line')}>{content}</li>
      ))}
    </div>
  )
};
